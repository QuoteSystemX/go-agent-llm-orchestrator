...
- **Full-Text Search**: Instant search across project logs, docs, and tasks via PostgreSQL `to_tsvector`/GIN indexing.
- **Data Retention**: Automatic background cleanup of old jobs and metrics.
- **Production Persistence**: a single PostgreSQL connection (`jackc/pgx/v5`) — migrated from three separate SQLite databases (main/search/metrics) in `fe9ee52` to fix concurrent-write contention. See `make db-up` (`docker-compose.yml`) for a local dev/test instance.
...
- `skills_load_file`: Load one sibling file belonging to a skill (e.g. a file under `scripts/`, `references/`, or loose at the skill root), as declared in that skill's `SKILL.md` `files:` frontmatter (comma-separated list). Callers read `SKILL.md` via `skills_load` first, parse its `files:` field themselves, then call this once per declared path — there is no directory-walk/auto-discover tool; file selection is static and author-declared by design.
...
- `ethics_veto`: `ethics-auditor` only — block a plan/task immediately. Lifting it requires council quorum (`risk-manager`, `cto`, `security-auditor`) via `council_vote`/`council_execute` on the auto-created lift-proposal; the vetoing ethics-auditor cannot vote on its own lift.
...
- `backup_s3`: Backup the PostgreSQL database to S3-compatible storage.
...
- PostgreSQL 14+ (a local instance for dev/test: `make db-up`, see `docker-compose.yml`)
...
| `-pg-url` | PostgreSQL connection URL (overrides `DATABASE_URL` env var). | *(required — one of the two must be set)* |
| `-retention` | Data retention period in days (for jobs and metrics). | `30` |
| `-index-dirs` | Comma-separated directories to index for full-text search. | `.agent,wiki,tasks` |
...
2. **Indexer (`indexer.go`)**: Integrates PostgreSQL's native `to_tsvector`/GIN full-text search for instant, high-performance searching across project documentation and logs. It monitors files via `fsnotify` for real-time updates.
3. **Persistence Layer (`db_*.go`)**: A modular set of database interfaces, all sharing one PostgreSQL connection:
   - **Governance**: Council proposals, voting, and audit trails.
   - **Ethics**: veto records — a block takes effect immediately on creation, lifting it goes through the same governance voting quorum (see `db_ethics.go`).

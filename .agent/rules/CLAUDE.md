...
2. **Source of truth is `.agent/`** — all changes go there first
3. **For new agents**: create in `.agent/agents/`, then run sync script
4. **For skill updates**: edit in `.agent/skills/`, then run sync script
5. **Distribution**: push to `main` → GitHub Actions auto-distributes to target repos

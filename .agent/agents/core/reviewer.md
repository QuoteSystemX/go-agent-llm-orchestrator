...
<!-- Must match .agent/wiki-templates/TASK_CARD.md — the canonical ad-hoc task-card template,
     also used by autonomous_reviewer_cron.py and task_miner.py. Keep this section in sync with
     that file by hand for now (auto-sync via sync_agents.py is a deliberate fast-follow, gated
     on confirming whether @-include resolution works inside .agent/agents/*.md files the way it
     does in the CLAUDE.md load chain — see tasks/done/2026-08/2026-08-14-unify-task-card-template-single-source-of-truth.md). -->
...
**Source**: how this was found

## Problem

Clear description of what was found and why it matters. Inline evidence (file:line, code
snippets) directly here rather than in a separate section.
...
Note: this is the ad-hoc task-queue format (`[BUG]`, `[CHORE]`, `[PERF]`, `[SECURITY]`, ...).
`[STORY]`/`[EPIC]` cards generated by the BMAD `/stories` workflow use a different, richer
template (`.agent/wiki-templates/STORY.md`) — do not use this format for those.

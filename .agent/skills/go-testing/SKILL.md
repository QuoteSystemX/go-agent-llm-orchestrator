---
name: go-testing
description: Production-ready Go tests — table-driven tests, testify suites and mocks, parallel tests, fuzzing, fixtures, goroutine leak detection with goleak, snapshot testing, code coverage, integration tests, idiomatic test naming. Use when writing or reviewing Go tests, choosing a testing approach, setting up Go test CI, or debugging flaky/slow tests. For testify-specific APIs see go-stretchr-testify.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash
version: 1.0.0
source: Adapted from samber/cc-skills-golang@golang-testing (MIT License) — https://github.com/samber/cc-skills-golang
---

# Go Testing Best Practices

Tests as executable specifications, not coverage-target filler.
...
12. Test files MUST be named after the source file under test, not after the function or method
13. Test functions SHOULD appear in the same order as the functions/methods they test
...
Name the test file after the source file it tests, not after the function under test — Go's convention is one test file per source file (`foo.go` → `foo_test.go`), because `go test`, coverage reports, and IDE navigation all resolve tests by source file, not by symbol.
...
Never create a testify `assert`/`require` instance in the parent test function and reuse it inside `t.Run` closures. `assert.New(t)` captures the exact `*testing.T` it was built with — failures raised inside the subtest get attributed to the *parent* test, and the failing subtest itself still reports `--- PASS`.
...
func TestCalculatePrice(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            is.Equal(tt.expected, CalculatePrice(tt.quantity, tt.unitPrice)) // misattributed
        })
    }
}
...
`testing/synctest` (Go 1.25+) provides deterministic tests for goroutines, timers, deadlines, and context cancellation. Time advances only when all goroutines are blocked.
...
Use `synctest.Test` in Go 1.25+/1.26+; the old Go 1.24 experimental `synctest.Run` API is a compatibility fallback only.
...
For Go 1.24+, new benchmarks should use `b.Loop()` instead of legacy `b.N` loops.
...
Many test best practices are enforced automatically: `thelper`, `paralleltest`, `testifylint`.
...
- go-stretchr-testify — detailed testify API (assert, require, mock, suite)
- go-database — database integration test patterns
- go-concurrency — goroutine leak detection with goleak

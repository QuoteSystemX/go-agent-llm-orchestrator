...
def sync(target: str, dry_run: bool = False, check: bool = False, only_agent: str = "", profile: str = "", no_commands: bool = False, no_workflows: bool = False, no_headroom: bool = False):
    ...
    # 4.5 Skills: symlink .agent/skills/<name>/ into .claude/skills/<name> so
    # they're real, natively-discovered Claude Code skills (see sync_skills
    # docstring) instead of just files agents self-instruct to Read.
    if not only_agent:
...
def sync_skills(target: str, dry_run: bool, check: bool):
    """Expose .agent/skills/<name>/ as native Claude Code skills.

    Claude Code only scans .claude/skills/<name>/SKILL.md (see
    https://code.claude.com/docs/en/skills) — .agent/skills/ was never on
    that path, so none of these 160+ knowledge modules were ever real Claude
    Code skills; agents just self-instruct to Read them.
    ...
    if skills_out is None:
        return  # target has no native skills directory (e.g. opencode)
